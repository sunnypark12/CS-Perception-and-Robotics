...
class Tests:
    ...
    def test_vision_only_func(self)->None:
        ...
        image1_data_path = ["Vision_only_world1_img_l.png", "Vision_only_world2_img_l.png"]
        image2_data_path = ["Vision_only_world1_img_r.png", "Vision_only_world2_img_r.png"]
        ...
        for i in range(2):
            student_depth, student_angle = vision_only_depth_calculation(cv2.imread(os.path.join(self.vision_data_path, image1_data_path[i])), 
                                                                            cv2.imread(os.path.join(self.vision_data_path, image2_data_path[i])),
                                                                            fov, translation)
            score_depth += self.check_dist_accuracy(student_depth, self.vision_solution[i]["depth"])
            score_angle += self.check_angle_accuracy(student_angle, self.vision_solution[i]["angle"])

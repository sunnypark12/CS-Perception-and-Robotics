...
def particle_likelihood(robot_marker_list: list[MarkerMeasure], particle_marker_list: list[MarkerMeasure]) -> float:
    ...
    matched_pairs, unmatched_robot_markers, unmatched_particle_markers = generate_marker_pairs(robot_marker_list, particle_marker_list)
    
    # Initialize likelihood
    likelihood = 1.0
    # Step 2: Compute the likelihood using matched markers.
    for robot_marker, particle_marker in matched_pairs:
        likelihood *= marker_likelihood(robot_marker, particle_marker)


    # Step 3: incorporate the likelihood of unmatched markers.
    for um in unmatched_robot_markers:
        likelihood *= compute_spurious_detection_rate(um)
    for up in unmatched_particle_markers:
        likelihood *= compute_detection_failure_rate(up)
...
def comptue_particle_weights(particles:list[SE2], robot_marker_measures:list[MarkerMeasure], env:Environment) -> list[float]:
    ...
    for particle in particles:
        # Predict marker measures for this particle
        particle_marker_measures = env.read_marker_measures(particle)
        # Calculate the particle's likelihood
    ...
    if total_weight > 0:
        ...
        normalized_weights = [1.0 / len(particle_weights) for _ in particle_weights]  # Handle case where no markers were observed
